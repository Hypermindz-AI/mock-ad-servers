...
export async function seedTikTok(): Promise<{
  campaigns: number;
  adGroups: number;
  ads: number;
  metrics: number;
}> {
  console.log('🌱 Seeding TikTok campaigns...');
  ...
  const ACCOUNT_ID = '7123456789012345678';
  ...
  // Check if TikTok campaigns already exist
  const existing = await campaignRepo.findByAccountId('tiktok', ACCOUNT_ID);
  if (existing.length > 0) {
    console.log(`   ℹ️  TikTok already has ${existing.length} campaigns, skipping`);
    return { campaigns: existing.length, adGroups: 0, ads: 0, metrics: 0 };
  }

  // Generate campaigns
  const campaigns = CampaignGenerator.generateBatch('tiktok', ACCOUNT_ID, CAMPAIGN_COUNT);
  ...
  for (const campaign of campaigns) {
    ...
    const adGroups = AdGroupGenerator.generateBatch('tiktok', createdCampaign.id, adGroupCount);
    ...
    for (const adGroup of adGroups) {
      ...
      const ads = AdGenerator.generateBatch('tiktok', createdCampaign.id, createdAdGroup.id, adCount);
  ...
  console.log(`   ✅ TikTok: ${CAMPAIGN_COUNT} campaigns, ${totalAdGroups} ad groups, ${totalAds} ads, ${totalMetrics} metrics`);
